...
const markup = images.reduce((html, product) =>{
  return (html += `
...
imagesListEl.insertAdjacentHTML('beforeend', markup);

// 4. Додали обробник події для кліку на картку
imagesListEl.addEventListener('click', onProductClick);

function onProductClick(event) {
  event.preventDefault();
  //if (event.target === event.currentTarget) return;
  
 const productCardEl = event.target.closest('.gallery-item');
